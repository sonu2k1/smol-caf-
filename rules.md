...
- **Backend:** Next.js API routes / Server Actions on top of Supabase — no separate Express server needed
- **Database:** **Postgres via Supabase** (ap-south-1 Mumbai region) — the SQL-vs-Mongo TODO is closed; Postgres was chosen because order/billing logic needs real multi-table transactions (see Rule 7 and 11)
- **Auth:** Supabase Auth for staff accounts, Row Level Security (RLS) as a second enforcement layer alongside server-side role checks — RLS is a backstop, not a replacement for Rule 10
...
/db                         → Supabase migrations, seed scripts (incl. menu-seed-master.csv import), query layer
...
- Money columns: integer, minor currency unit (paise) — never decimal/float — e.g. `total_paise`, `unit_price_paise`
...
- Never trust client-side price totals — recalculate server-side before saving an order, inside one DB transaction
- Order creation must accept a client-generated idempotency key — a retried/double-tapped request with the same key must return the original order, never create a second one
- Order status transitions validated server-side (can't skip from `received` to `ready` directly unless explicitly allowed)
- Order line items store a **snapshot** of item name/price at order time — never join to the live `menu_items` price for a completed order, so editing the menu later can't change a past receipt

## 8. Real-Time / Live Updates

- Kitchen queue and order status need near-real-time updates — start with **polling** (3–5s, simplest, fine for a single cafe); move to Supabase Realtime/WebSockets only if polling feels laggy in practice
...
- Staff auth via Supabase Auth (handles password hashing) — don't hand-roll bcrypt/session logic
- Role-based route protection on every staff route (server-side, not just UI hiding) — RLS policies as a second layer, not the only layer
...
- The live menu is v1-seeded once from the smol café V0.8 workbook (`menu-seed-master.csv`, 59 items) via a dry-run-then-apply import script — after that, Admin CRUD is the only way the menu changes
- Never re-read the spreadsheet at runtime and never hardcode menu items into components — menu is data, not markup

## 13. Testing (lightweight, solo project)

- Manual test checklist per role before each deploy (see phases.md)
- Priority for actual unit tests: order total/tax/discount calculation, idempotent order creation, order status transition logic, shelf-life status derivation — these are the highest-risk-of-bug areas

## 14. AI-Assistant Specific Rules

- Always check `memory.md` for current project state before suggesting changes
- Stack is locked (Rule 1) — don't re-propose a different DB or real-time approach without flagging it first
